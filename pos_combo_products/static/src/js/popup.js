...
odoo.define('pos_combo_products.popups', function (require) {
    "use strict";
    const AbstractAwaitablePopup = require('point_of_sale.AbstractAwaitablePopup');
    const Registries = require('point_of_sale.Registries');
    const { onMounted } = owl;

    class ComboPopupWidget extends AbstractAwaitablePopup {
        setup() {
            super.setup();
        ...
        onMounted() {
            ...
            $('.grid_product').click(function (e) {
                ...
                var product = self.env.pos.db.all_combo_products_by_id[prod_id];
            ...
            if (line) {
                ...
                // list
                if (self.env.pos.config.combo_popup_view == 'list') {
            ...
            $('.wk_qty_product').on('input', function (e) {
                ...
                tab_groups.each(function (i, elem) {
                    ...
                    var group_data = self.env.pos.db.all_combo_groups_by_id[tab_id];
                    ...
                    if (self.env.pos.config.combo_popup_view == "list") {
                        ...
                        if (qty > group_data.maximum_combo_products) {
                            if(grid_last_selection){
                                ...
                                var product = self.env.pos.db.all_combo_products_by_id[grid_last_selection.attr('data-id')]
                                if(product){
                                    if (product.manage_inventory) {
        ...
        wk_click_confirm() {
            ...
            var qty_combo_dict = {};
            var line = self.props.line;
            var combo_products = self.env.pos.db.all_combo_products_by_id;
            var sel_combo_prod = []
            ...
            if (Object.keys(minimum_qty_dict).length > 0) {
                self.showPopup("MinQtyPopupWidget", {
                ...
                var product = self.env.pos.db.product_by_id[self.props.product.id]
    ...
    ComboPopupWidget.template = 'ComboPopupWidget';
    Registries.Component.add(ComboPopupWidget);

    class MinQtyPopupWidget extends AbstractAwaitablePopup {
        click_min_comfirm() {
            this.cancel();
        }
    }
    MinQtyPopupWidget.template = 'MinQtyPopupWidget';
    Registries.Component.add(MinQtyPopupWidget);
});
